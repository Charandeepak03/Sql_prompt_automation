...
- Target database:Charan_sols.db
- Table:Charan_sols.
...
A: SELECT COUNT(*) FROM Charan_sols

Q: Show all Data Engineers
A: SELECT * FROM Charan_sols WHERE employee_role = 'Data Engineer'

Q: Who earns more than 60000?
A: SELECT * FROM Charan_sols WHERE employee_salary > 60000

Q: Who earns the highest salary?
A: SELECT * FROM Charan_sols ORDER BY employee_salary DESC LIMIT 1
...
st.title("Gemini SQL Assistant (Charan sols company DB)")
st.write("Ask questions in English. Get SQL queries and results instantly!")

# User input
question = st.text_input("Enter your question whenever:")
...
if st.button("Run"):
    if question.strip() == "":
        ...
        # Run SQL query
        result, columns = read_sql_query(sql_query, "Charan_sols.db")
